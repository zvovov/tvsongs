...
if __name__ == "__main__":
    cont_season = 'y'
    _season = 1
    _name = getSlug("game of thrones")
    
    soup = getSoup(_base_url, _type, _name, _season, "")

    _season_count = soup.find_all('ul')[1]
    _season_count = (_season_count.find_all('li')[-1].find('a')['href'])[-1]
    ...
    while cont_season == 'y' or cont_season == 'Y':
        ...
        _season_start = 0 if _season_wanted == 0 else _season_wanted
        _season_end = _season_count if _season_wanted == 0 else _season_wanted + 1

        for curr_season in range(_season_start, _season_end):
            curr_soup = getSoup(_base_url, _type, _name, curr_season, "")
            _lists = curr_soup.find_all('li')
            _episode_count = 0
            _link_dict = {}
            for i in _lists:
                if i.find('h3'):
            ...
            _episode_start = 0 if _episode_wanted == 0 else _episode_wanted
            _episode_end = _episode_count if _episode_wanted == 0 else _episode_wanted + 1

            for curr_episode in range(_episode_start, _episode_end):
                episode_soup = getSoup(_base_url, _type, _name, curr_season, _link_dict[curr_episode])
